...
class RoboDesk(gym.Env):
    ...
    def __init__(self, config, **kwargs):
        ...
        # for xc
        self.max_episode_steps = self.episode_length
    ...
    def render(self, mode='rgb_array', resize=True):
        ...
        if resize:
            ...
            image = cv2.resize(np.asarray(image), [self.image_size, self.image_size], interpolation=cv2.INTER_AREA)
    ...
    def step(self, action):
        ...
        return self._get_obs(), total_reward, done, done, {'discount': 1.0}
    ...
    def _get_obs(self):
        if self.config.pixel:
            return self.render(resize=True)
        else:
